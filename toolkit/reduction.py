...
def photometry(image_paths, master_dark_path, master_flat_path, star_positions,
    ...
    with ProgressBar(len(image_paths)) as bar:
        for i in range(len(image_paths)):
            ...
            #     plt.scatter(brightest_star_coords[1], brightest_star_coords[0])
            #     plt.show()
            ...
            for j in range(len(star_positions)):
                ...
                #x_stamp_centroid, y_stamp_centroid = centroid_com(image_stamp - np.median(image_stamp))
                #x_stamp_centroid, y_stamp_centroid = centroid_1dg(image_stamp - np.median(image_stamp))

                #x_stamp_centroid, y_stamp_centroid = centroid_com(gaussian_filter(image_stamp - np.median(image_stamp), 3))
                # x_stamp_centroid, y_stamp_centroid = np.unravel_index(np.argmax(gaussian_filter(image_stamp - np.median(image_stamp), 2)),
                #                                                                 image_stamp.shape)
                ...
                #     print(y_stamp_centroid, x_stamp_centroid)
                #     plt.plot(y_stamp_centroid, x_stamp_centroid, '.')
                #     plt.show()
